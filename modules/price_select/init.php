<?php
/*
* Define class aiowaffPriceSelect
* Make sure you skip down to the end of this file, as there are a few
* lines of code that are very important.
*/
!defined('ABSPATH') and exit;

if (class_exists('aiowaffPriceSelect') != true) {
    class aiowaffPriceSelect
    {
        /*
        ...
        */
        public function __construct()
        {
			...
			//$this->settings = $aiowaff->getAllSettings('array', 'amazon');
			...
			add_action('wp_ajax_aiowaffPriceSelectSave', array( $this, 'ajax_request' ));
        }
		
		public function admin_load_styles()
		{
			wp_enqueue_style( 'aiowaff-price-select-styles', $this->module_folder . 'app.css' );
		}
		
		public function admin_load_scripts()
		{
			wp_enqueue_script( 'aiowaff-price-select-scripts', $this->module_folder . 'app.class.js' );
		}
		...
		public function add_product_inline( $post_id = 0 )
		{
			...
			foreach ( $arrProducts as $post_id ) { // products loop!
				...
				// start html
				$html[] = '<div class="aiowaffPriceSelectHidden" data-post_id="' . ($post_id) . '" style="display: none;">';
				...
				foreach (array('regular', 'sale') as $priceType) {
					...
					foreach (array('auto', 'selected', 'ancestry', 'current') as $metaVal) {
						...
						$attr = array(
							'cssClass'			=> "aiowaff-price-$priceType-$metaVal",
							'name'				=> "aiowaff-price[$post_id][$priceType][$metaVal]",
							'value'				=> $_value,
						);
				...
				// is amazon product!
				$attr = array(
					'cssClass'			=> "aiowaff-price-isprodamz",
					'name'				=> "aiowaff-price[$post_id][isprodamz]",
				...
				$html[] = '<div id="aiowaffPriceSelectInline-' . ($post_id) . '" style="display: none;">';
				$html[] = 	'<div class="aiowaffPriceSelectWrapper" data-post_id="' . ($post_id) . '">';
				
				$buttons = $this->build_buttons();
				$html[] = $buttons;
				
				$html[] = 		'<div id="aiowaffPriceSelect">';
		...
		public function build_buttons() {
			ob_start();
		?>
			<div class="aiowaffPriceSelect-buttons">
			...
			if ( is_array($response) ) {
				...
				if ( !empty($isPriceTag) ) {
					...
					if ( !empty($price['Amount']) ) {
						...
						// aiowaffPriceSelect-Ancestry-' . $resp_key . '
						$arr[] = '<ul class="aiowaff-priceselect-price">';
						$arr[] = 	'<li>';
						...
						$arr[] = 			'<label for="aiowaff-price-regular-'.($price_attr['id']).'" class="aiowaff-price-regular">' . 'regular' . '</label><input type="radio" id="aiowaff-price-regular-'.($price_attr['id']).'" name="aiowaff-price-regular['.($post_id).']" class="aiowaff-price-regular" />';
						$arr[] = 			'<label for="aiowaff-price-sale-'.($price_attr['id']).'" class="aiowaff-price-sale">' . 'sale' . '</label><input type="radio" id="aiowaff-price-sale-'.($price_attr['id']).'" name="aiowaff-price-sale['.($post_id).']" class="aiowaff-price-sale" />';
						$arr[] = 		'</span>';
						$arr[] = 	'</li>';
						$arr[] = '</ul>';
					}
				...
				foreach ($response as $key => $tag) {
					...
					$arr[] = '<ul class="aiowaffPriceSelect-Ancestry-' . $key . '">';
		...
		public function save_post() {
			...
			$req = array(
				'price'			=> isset($_REQUEST['aiowaff-price']) ? $_REQUEST['aiowaff-price'] : array()
			);
			extract($req);
		...
		private function page_list_buttons( $post_id ) {
			...
			$html[] = '
			<div class="aiowaffPriceSelectButtons" data-post_id="' . ($post_id) . '">
				<ul>
					<li class="_aiowaff_regular_price_field">
						<label for="_aiowaff_regular_price">Regular Price:</label>
						<input id="_aiowaff_regular_price" class="_aiowaff_regular_price short wc_input_price" type="text" placeholder="" name="_aiowaff_regular_price" value="' . $_regular_price . '" readonly="readonly">
					</li>
					<li class="_aiowaff_sale_price_field">
						<label for="_aiowaff_sale_price">Sale Price:</label>
						<input id="_aiowaff_sale_price" class="_aiowaff_sale_price short wc_input_price" type="text" placeholder="" name="_aiowaff_sale_price" value="' . $_sale_price . '" readonly="readonly">
					</li>
				</ul>
			</div>';
		...
		public function _edit_columns($columns) {
			$new_columns['aiowaff_price'] 			= __('aiowaff Price', $this->the_plugin->localizationName);
...
?>
<style type='text/css'>
	th#aiowaff_product_info { width: 170px; }
	
	.aiowaff_product_info .aiowaffPriceSelectButtons {
	}
	.aiowaff_product_info .aiowaffPriceSelectButtons ul {
		display: inline-block;
		margin: 4px 0px;
	}
	.aiowaff_product_info .aiowaffPriceSelectButtons ul li {
		display: inline-block;
	}
	.aiowaff_product_info .aiowaffPriceSelectButtons ul li > label {
		display: inline-block;
		width: 85px;
	}
	.aiowaff_product_info .aiowaffPriceSelectButtons input {
		width: 70px;
	}
	.aiowaff_product_info .aiowaffPriceSelectButtons .aiowaff-priceselect-wrapper {
		float: none;
		margin-left: 0px;
	}
</style>
<?php
	    ...
	    * Singleton pattern
	    *
	    * @return aiowaffPriceSelect Singleton instance
	    */
...
// Initialize the aiowaffPriceSelect class
$aiowaffPriceSelect = aiowaffPriceSelect::getInstance();