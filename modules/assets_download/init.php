<?php
/*
* Define class aiowaffAssetDownload
* Make sure you skip down to the end of this file, as there are a few
* lines of code that are very important.
*/
!defined('ABSPATH') and exit;

if ( !class_exists('aiowaffAssetDownload') ) {
    class aiowaffAssetDownload
    {
        /*
        ...
        */
        public function __construct()
        {
			...
			add_action('wp_ajax_aiowaff_download_asset', array( &$this, 'ajax_download_asset' ));
			add_action('wp_ajax_aiowaffDeleteAssetsProducts', array( &$this, 'delete_products_asset' ));
			//$this->__test_assets();
        }
	    ...
	    * Singleton pattern
	    *
	    * @return aiowaffAssetDownload Singleton instance
	    */
...
?>
		...
		<div id="aiowaff-wrapper" class="fluid wrapper-aiowaff aiowaff-asset-download">
			
			<?php
			// show the top menu
			aiowaffAdminMenu::getInstance()->make_active('info|assets_download')->show_menu();
			?>
			...
			<div id="aiowaff-content">
				...
				<h1 class="aiowaff-section-headline">
					...
					if( isset($this->module['assets_download']['in_dashboard']['icon']) ){
						echo '<img src="' . ( $this->module_folder . $this->module['assets_download']['in_dashboard']['icon'] ) . '" class="aiowaff-headline-icon">';
					}
					?>
					<?php echo $this->module['assets_download']['menu']['title'];?>
					<span class="aiowaff-section-info"><?php echo $this->module['assets_download']['description'];?></span>
					<?php
						...
						<div id="aiowaff-content-area">
                            ...
                            <div class="aiowaff-grid_4">
	                        	<div class="aiowaff-panel">
									...
									<div class="aiowaff-panel-content">
										<div class="aiowaff-sync-details">
											...
											<p><img width="510" height="192" class="aligncenter size-full wp-image-81" alt="wpcron-cpanel" src="' . ($this->the_plugin->cfg['paths']['plugin_dir_url'] . 'modules/synchronization/') . 'assets/wpcron-cpanel.png"></p>
											...
											<p><img width="470" height="331" class="aligncenter size-full wp-image-82" alt="wpcron-add-new-cron-job" src="' . ($this->the_plugin->cfg['paths']['plugin_dir_url'] . 'modules/synchronization/') . '/assets/wpcron-add-new-cron-job.png"></p>
...
<?php
		...
		*/
		public function ajax_request()
		{
			...
			$asin = $this->the_plugin->get_post_meta($request['id'], $key, true);
		...
		private function get_asset_generic( $asset_id=0, $nb_dw='all', $post_id=0, $include_variations=true, $inprogress=false, $include_err=false, $include_invalid_post=false ) {
            ...
            if ( !$include_invalid_post ) {
                $q = "select a.id, a.post_id, a.asset, a.thumb, a.download_status, a.hash, a.media_id, b.title from " . $tables['assets'] . " as a left join " . $tables['products'] . " as b on a.post_id = b.post_id left join " . $tables['posts'] . " as c on b.post_id = c.ID where 1=1 and !isnull(b.post_id) and !isnull(c.ID) $__q_dw ";
            }
		...
		private function upd_asset_db( $asset, $first_item=false ) {
			...
			if ( isset($dwimg['attach_id']) && $dwimg['attach_id'] > 0 ) { // image was downloaded and inserted as media in wp posts
				...
				$file_type = 'image';
				$file_ext = $this->the_plugin->u->fileExtension( $asset->asset );
				//preg_match('/(jpg|jpe|jpeg|gif|png)/ui', $file_ext, $m);
				if ( $__tmp = preg_match('/mp3$/', $file_ext) ) {
					$file_type = 'audio';
				}
				
				if ( 'audio' == $file_type ) {
					$provider = get_post_meta($post_id, '_aiowaff_prodtype', true);
					if ( 'envato' == $provider ) {
						update_post_meta( $post_id, '_we_audio_preview', wp_get_attachment_url( $dwimg['attach_id'] ) );
					}
				}
...
/*if ( !function_exists('aiowaffAssetDownload_cronjob') ) {
function aiowaffAssetDownload_cronjob() {
	// Initialize the aiowaffAssetDownload class
	$amzaffAssetDownload = new aiowaffAssetDownload();
	$amzaffAssetDownload->cronjob();
}
}*/

// Initialize the aiowaffAssetDownload class
$aiowaffAssetDownload = aiowaffAssetDownload::getInstance();