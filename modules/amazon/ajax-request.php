...
add_action('wp_ajax_aiowaff_load_product', 'aiowaff_load_product_callback');
function aiowaff_load_product_callback() {
	global $aiowaff;
	...
	require_once( $aiowaff->cfg['paths']['plugin_dir_path'] . '/lib/scripts/amazon/aaAmazonWS.class.php');
    ...
    $aaAmazonWS->set_the_plugin( $aiowaff );
	...
	if($product['Items']["Request"]["IsValid"] == "True"){
		...
		if(count($product['Items']['Item']) > 0){
			...
			$prod_id = $aiowaff->addNewWooProduct($retProd);
...
add_action('wp_ajax_amazon_request', 'aiowaffamazon_request_callback');
function aiowaffamazon_request_callback() {
	global $aiowaff;
	...
	?>
			<div class="resultsTopBar">
				<h2>
					Showing <?php echo $requestData['page'];?> - <?php echo $response['Items']["TotalPages"];?> of <span id="aiowaff-totalPages"><?php echo $response['Items']["TotalResults"];?></span> Results
				</h2>

				<div class="aiowaff-pagination">
					<span>View page:</span>
					<select id="aiowaff-page">
						<?php
						for( $p = 1; $p <= 5; $p++ ){
						...
						<td class="product-options">
							<?php
								if($value['CustomerReviews']['HasReviews'] == true){
									echo '<a class="thickbox aiowaff-option-btn" href="' . ( $value['CustomerReviews']['IFrameURL'] ) . '&TB_iframe=true"> <img src="'. ( $plugin_uri ) .'assets/comments.png" /> Customer Reviews</a>';
								}
							?>
							<a class="aiowaff-option-btn aiowaff-load-product" href="#" rel="<?php echo $value['ASIN'];?>"><img src="<?php echo $plugin_uri;?>assets/update.png" /> Load this product </a>
						<td>
					</tr>
		<?php
				} // end foreach
