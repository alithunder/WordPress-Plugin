...
class amzAffReviewProductTab {
	...
	public function __construct() {
		...
		$this->tab_title = __('Amazon Customer Reviews', 'wooallinone');
			...
			if( trim($url) != "" ){
				...
				if( trim($params['exp']) != "" ){
					...
					if( time() > $expire_on ){
						...
						if( $post_id > 0 ){
							$new_url = $aiowaff->amzHelper->updateProductReviews( $post_id );
	...
	private function product_has_custom_tabs($product) {
		...
		if($this->tab_data === false) {
			...
			$this->tab_data[] = isset($reviews, $reviews[0]) ? $reviews[0] : array('content' => '');
			 
			if( isset($cross_selling) && count($cross_selling) > 1 ){
				$this->tab_data[] = array(
					'id' => $aiowaff->alias . '_cross_selling_tab',
