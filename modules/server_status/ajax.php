...
if (class_exists('aiowaffServerStatusAjax') != true) {
    class aiowaffServerStatusAjax extends aiowaffServerStatus
    {
        ...
        */
        public function __construct( $the_plugin=array() )
        {
			...
			// ajax  helper
			add_action('wp_ajax_aiowaffServerStatusRequest', array( $this, 'ajax_request' ));
		}
		...
		*/
		public function ajax_request()
		{
			...
			if( in_array( 'check_memory_limit', array_values($actions)) ){
				...
				$html = array();
            	if ( $memory < 127108864 ) {
            		$html[] = '<div class="aiowaff-message aiowaff-error">' . sprintf( __( '%s - We recommend setting memory to at least 128MB. See: <a href="%s">Increasing memory allocated to PHP</a>', $this->the_plugin->localizationName ), size_format( $memory ), 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP' ) . '</div>';
            	} else {
            		$html[] = '<div class="aiowaff-message aiowaff-success">' . size_format( $memory ) . '</div>';
            	}
			...
			if( in_array( 'export_log', array_values($actions)) ){
				...
				header( 'Content-Disposition: attachment; filename="aiowaff-logs.html"' );
			...
			if( in_array( 'remote_get', array_values($actions)) ){
				...
				$return = array(
					...
					'html' 		=> ( $status == true ? '<div class="aiowaff-message aiowaff-success">' : '<div class="aiowaff-message aiowaff-error">' ) . $msg . '</div>' 
				);
        	}
					...
					if ( ! empty( $plugin_data['Name'] ) ) {
						...
						if ( strstr( $dirname, $this->the_plugin->localizationName ) ) {
							...
							if ( false === ( $version_data = get_transient( $plugin . '_version_data' ) ) ) {
								$changelog = wp_remote_get( '' . $dirname . '' );
