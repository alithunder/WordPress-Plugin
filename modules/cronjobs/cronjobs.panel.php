...
if(class_exists('aiowaffCronjobsPanel') != true) {

    class aiowaffCronjobsPanel {
        /*
        ...
        * Singleton pattern
        *
        * @return aiowaffCronjobs Singleton instance
        */
        ...
        public function test_cron_spawn( $cache = true ) {
            ...
            $cached_status = get_transient( 'aiowaff-cronjobs-test-ok' );
            ...
            if ( is_wp_error( $result ) ) {
                ...
                set_transient( 'aiowaff-cronjobs-test-ok', 1, 3600 );
                return true;
            }
        ...
        public function show_cron_status() {
            ...
            if ( is_wp_error( $status ) ) {
                $msg = sprintf( __( 'Issue encountered when trying to spawn a call to the WP-Cron system on your website. The WP-Cron jobs on your website may not work. The issue details: %s', 'aiowaff' ), '<br /><strong>' . esc_html( $status->get_error_message() ) . '</strong>' );
                $__status = false;
            } else {
                $msg = sprintf( __( 'Successfully spawn a call to the WP-Cron system on your website. The WP-Cron jobs on your website seems to work fine.', 'aiowaff' ) );
            }
            
            if (1) {
                ob_start();
                ?>
        <div class="aiowaff-form-row">
            <div class="aiowaff-message aiowaff-<?php echo $__status ? 'success' : 'error'; ?>">
                <p><?php echo $msg; ?></p>
            </div>
        </div>
                <?php
        ...
        public function show_timezone() {
            ...
            $html = array();
            $html[] = sprintf( __( 'Local timezone is <strong>%s</strong>', 'aiowaff' ), '<code>' . $tz . '</code>' );
            $html[] = '<span id="aiowaff-utc-time">' . sprintf( __( 'UTC time is <strong>%s</strong>', 'aiowaff' ), '<code>' . date_i18n( $time_format, false, true ) . '</code>' ) . '</span>';
            $html[] = '<span id="aiowaff-local-time">' . sprintf( __( 'Local time is <strong>%s</strong>', 'aiowaff' ), '<code>' . date_i18n( $time_format ) . '</code>' ) . '</span>';
        ...
        public function get_cron_events() {
            ...
            if ( empty( $crons ) ) {
                return array_merge($ret, array('msg' => __('You currently have no scheduled cron events.', 'aiowaff')));
            }
            ...
            return array_merge($ret, array(
                'status'            => 'valid',
                'msg'               => sprintf( __('You have %s scheduled cron events.', 'aiowaff'), count($events) ),
                'events'            => $events,
            ));
        }
        ...
        public function get_plugin_cron_events( $retType='return' ) {
            ...
            foreach( $plugin_crons as $cron_id => $cron ) {
                ...
                $args = empty( $event->args ) ? '<em>' . __( 'None', 'aiowaff' ) . '</em>' : json_encode( $event->args );
                $next_run = empty($event->time) ? '<em>' . __( 'None', 'aiowaff' ) . '</em>' : get_date_from_gmt(date('Y-m-d H:i:s', $event->time), $time_format)." (".$this->time_since($current_time, $event->time).")";
                $recurrence = empty($event->time) ? '<em>' . __( 'None', 'aiowaff' ) . '</em>' :  ( $event->schedule ? $this->interval($event->interval) : __('Non-repeating', 'aiowaff') );
                $status = $cron['status'];
                $duration = isset($cron['run_duration']) && $cron['run_duration'] != '' ? sprintf( __('%s seconds', 'aiowaff'), $cron['run_duration'] ) : '';
                
                $buttons = '<input type="button" class="aiowaff-button ' . ($cron['is_active'] == 'yes' ? 'red' : 'green') . '" value="' . ($cron['is_active'] == 'yes' ? __( 'Clear', 'aiowaff' ) : __( 'Activate', 'aiowaff' )) . '" style="width:122px">';
        ...
        public function printListInterface ()
        {
            ...
            $html = array();
            $html[] = '<style>#aiowaff-amazonDebug { display: block } </style>';
            
            $html[] = '<script type="text/javascript" id="aiowaff-cronjobs-js" src="' . ( $this->module['folder_uri'] ) . 'app.cronjobs.js" ></script>';
            $html[] = "<link rel='stylesheet' id='aiowaff-cronjobs-css' href='" . ( $this->module['folder_uri'] ) . "app.cronjobs.css" . '?'.time() . "' type='text/css' media='all' />";

            ob_start();
        ?>

<?php /*<div class="aiowaff-form-row">
    <div class="aiowaff-form-item large">
        <span class="formNote"><?php _e('Cronjobs setted by our plugin.', 'aiowaff'); ?></span>
    </div>
</div>*/ ?>
<?php
...
?>
<div class="aiowaff-form-row aiowaff-cj-list">
    ...
    <div id="aiowaff-cj-reload">
        <input type="button" class="aiowaff-button blue" value="<?php _e( 'Reload cronjobs list', 'aiowaff' ); ?>" style="width:182px">
    </div>
    <table id="aiowaff-cj-table">
        <thead>
            <tr>
                <th><?php _e('Hook Name', 'aiowaff'); ?></th>
                <th><?php _e('Arguments', 'aiowaff'); ?></th>
                <th><?php _e('Next Run', 'aiowaff'); ?></th>
                <th><?php _e('Recurrence', 'aiowaff'); ?></th>
                <th><?php _e('Last status', 'aiowaff'); ?></th>
                <th><?php _e('Last duration', 'aiowaff'); ?></th>
                <th><?php _e('Action', 'aiowaff'); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php 
...
<div class="aiowaff-form-row aiowaff-cj-help">
    <?php
    //$cron_url = $this->the_plugin->cfg["paths"]["plugin_dir_url"] . 'do-cron.php';
    $cron_url = site_url( 'wp-cron.php?doing_wp_cron' );
    echo '<h1>How to Replace WordPress Cron With A Real Cron Job</h1>
    ...
    ?>
        ...
        public function interval( $since ) {
            ...
            $chunks = array(
                array(60 * 60 * 24 * 365 , _n_noop('%s year', '%s years', 'aiowaff')),
                array(60 * 60 * 24 * 30 , _n_noop('%s month', '%s months', 'aiowaff')),
                array(60 * 60 * 24 * 7, _n_noop('%s week', '%s weeks', 'aiowaff')),
                array(60 * 60 * 24 , _n_noop('%s day', '%s days', 'aiowaff')),
                array(60 * 60 , _n_noop('%s hour', '%s hours', 'aiowaff')),
                array(60 , _n_noop('%s minute', '%s minutes', 'aiowaff')),
                array( 1 , _n_noop('%s second', '%s seconds', 'aiowaff')),
            );
    
    
            if( $since <= 0 ) {
                return __('now', 'aiowaff');
            }
            ...
            // set output var
            $output = sprintf(_n($name[0], $name[1], $count, 'aiowaff'), $count);
            ...
            if ($i + 1 < $j)
                ...
                if (($count2 = floor(($since - ($seconds * $count)) / $seconds2)) != 0)
                    {
                    // add to output var
                    $output .= ' '.sprintf(_n($name2[0], $name2[1], $count2, 'aiowaff'), $count2);
                    }
                }
...
//$aiowaffCronjobsPanel = new aiowaffCronjobsPanel($this->cfg, $module);
//$aiowaffCronjobsPanel = aiowaffCronjobsPanel::getInstance();