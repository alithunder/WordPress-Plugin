...
if (class_exists('aaTeamPluginDepedencies') != true) {
    class aaTeamPluginDepedencies
    {
	    ...
	    * Singleton pattern
	    *
	    * @return aiowaffSpinner Singleton instance
	    */
		...
		public function createDepedenciesPage() {
			add_menu_page(
				...
				$this->the_plugin->cfg['paths']['plugin_dir_url'] . ''
			);
		}
		...
		public function depedencies_plugin_redirect_valid() {
			//delete_option('aiowaff_depedencies_is_valid');
		...
		public function depedencies_plugin_redirect() {
			...
			delete_option('aiowaff_depedencies_do_activation_redirect');
			wp_redirect( get_admin_url() . 'admin.php?page=aiowaff' );
		}
		...
		public function verifyDepedencies() {
			...
			?>
			<input type="button" value="Re-Check Depedencies" class="aiowaff-button blue aiowaff-depedencies-check" style="margin-top: 10px;">
			<?php
